...
class GroupBlock(TimmBlock):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        #print(drop_path, act_layer, attn_drop)
...
class PyramidVisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ###
        x = 255.0 * x
        resized_x = torch.zeros((x.shape[0], x.shape[1], 224, 224))
        for j in range(resized_x.size(dim = 0)):
            resized_x[j] = 255.0 * TF.resize(img = x[j], size = (224, 224))
        x = resized_x.to('cuda')
        ###
...
class ALTGVT(PCPVT):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256],
                 num_heads=[1, 2, 4], mlp_ratios=[4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[4, 4, 4], sr_ratios=[4, 2, 1], block_cls=GroupBlock, wss=[7, 7, 7], pretrained_cfg = None, pretrained_cfg_overlay = None):
